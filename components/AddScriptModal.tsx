...
export const AddScriptModal: React.FC<AddScriptModalProps> = ({ isOpen, onClose, onAddScript, categories, subCategories }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-zinc-900 rounded-xl border border-zinc-700 w-full max-w-2xl shadow-2xl relative p-8" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-zinc-800/50 text-zinc-200 border border-zinc-700 rounded-md p-2.5 focus:ring-2 focus:ring-orange-500 focus:outline-none transition-all"
              required
            />
          </div>
          <div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={6}
              className="w-full bg-zinc-800/50 text-zinc-200 border border-zinc-700 rounded-md p-2.5 font-mono text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none transition-all"
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2 text-sm font-semibold text-zinc-300 bg-zinc-800/50 border border-zinc-700 rounded-md hover:bg-zinc-700 transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-6 py-2 font-semibold text-white bg-orange-600 rounded-md hover:bg-orange-500 transition-colors"
            >
              Save Script
            </button>
          </div>
